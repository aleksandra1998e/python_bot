...
def new_city_keyboards(response: json) -> types.InlineKeyboardMarkup:
...
def check_city(city: str, bot: telebot.TeleBot, id: int) -> bool:
    ...
    response = requests.request("GET", url, headers=headers, params=querystring_first, timeout=(3, 30))
    ...
    if response_json["moresuggestions"] != 0:
        ...
        response = requests.request("GET", url, headers=headers, params=querystring_second, timeout=(3, 30))
        response_json = json.loads(response.text)
        if response_json["moresuggestions"] != 0:
            keyboard = new_city_keyboards(response_json)
            bot.send_message(
